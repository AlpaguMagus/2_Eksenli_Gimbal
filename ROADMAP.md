...
| **K1** · 🔧 firmware + 🧪 HP tek-eksen valide | 2-eksen decentralized cascade | 2. ekseni entegre | **HP loop-rate fix GEÇİLDİ** (`GPIO_PULLUP`, §12.13 — timer-ISR DEĞİL); sıradaki: 2-eksen birlikte + LP rijit re-do (LP+IMU bağlanınca) | HP gross stick-slip çözüldü (residual limit-cycle→K7); 2-eksen cascade gimbal | 3.3 |
...
- **3.4 — K4/MIMO sistem tanımlama:** her motoru ayrı sür / diğer ekseni ölç → 2×2 $G(s)$ (`tfest`). ⚠ **Donanım ASİMETRİK** — HP (Motor1, 20:1) ve LP (Motor2, 9.7:1) farklı redüktör/Kg → eksenler **özdeş değil**; her eksen **ayrı karakterize** edilir (HP zaten `§12.12`'de: Kg=1043 serbest-mil rad/s/duty, rijit re-char §12.13.5 ~974/897, τ≈72ms; LP Aşama-1/2 modeli). Eski "motorlar nominal özdeş → $G_{22}$ bedava" varsayımı bu asimetriyle **geçersiz**. ⚠ 2 motor mekanik bağlı olmadan çapraz terimler ~0 → anlamlı kuplaj yüklü gimbalda
