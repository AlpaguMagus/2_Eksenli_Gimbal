...
**Şu an:** 🟡 **Aşama 3 (MIMO) AÇIK** (`feature/asama-3-mimo-model`, 2026-06-09). **3.1 pin planı ✅ onaylı + kablolama tamam; 3.2a encoder-2 bench PASS** (`artifacts/3/enc2_test/`); IMU kendini-iyileştirme firmware. **3.2b motor-2 sürücü ✅ firmware + bench PASS** (`Motor2_*` minimal açık-döngü, `DUTY2:`+`U2`; bench: motor-2 ±0.30'da +1203/−1199 count/s, **polarite +duty→+count = motor-1 ile AYNI** → 3.3 cascade işaret çevirme YOK; motor-2 stall + shared-struct refactor 3.3'e ertelendi, sertifikalı motor-1 dokunulmadı). **⚠ Bench bulgusu:** rewire'da roller değişti — karakterize sağlıklı ünite şimdi **motor-2**; **motor-1 fiziksel ünitesinde yöne-bağlı mekanik kusur** (CCW serbest, CW'de 0.50 duty'de bile takılır, elle de zor; görünür engel yok → gearbox asimetri). Kullanıcı kararı (2026-06-09): olduğu gibi devam, 3.4'te asimetrik $G_{11}$ karakterize. **Sıradaki: baseline 2-eksen (3.3)** — Aşama-2 cascade'i her eksene (işaret çevirme yok) + motor-2 stall. Aşama-2-sonrası paket main'de (`512e796`): mirror Kp_pos=6 gerçek 4.02°, amper bütçesi datasheet denetimi (3A yeterli, dar boğaz=sürücü), **count-tabanlı stall fix bench PASS** (yanlış-pozitif 0, oto-devam ~1 sn — 2A.T5-B kapandı), IMUDIAG/IMUINIT teşhis komutları. Önceki: ✅ Aşama 1+2 KAPALI (`asama-2-kapali` tag). Aşama 1: `K=53.89 rad/s/V, τ_median=60.5 ms, V_dead≈0, V_supply=12.15V`. Aşama 2.1: 5 kontrolcü tasarım. Aşama 2.2: firmware Tustin PI + anti-windup + MODE/SP_W komutları.
