...
**Şu an:** 🟡 **Aşama 3 (MIMO) AÇIK** (`feature/asama-3-mimo-model`). Asimetrik donanım: **HP** (Motor1/HW-039/BTS7960, 20:1, yük) + **LP** (Motor2/TB6612, 9.7:1). **LP cascade çalışıyor** (K0 kapandı — Test 2.5/MIRROR/STAB). **HP karakterize + analitik cascade tasarlandı** (Kg=1043, τ≈70ms, forward temiz) ama **bench STICK-SLIP** → "~32ms loop kök-neden" sanıldı. **✅ (1) ÇÖZÜLDÜ (§12.13):** 32ms **kopuk-IMU I2C BUSY-timeout ARTEFAKTIYDI** (IMU hiç bağlı değildi → bus float → BUSY-flag stuck → her okuma 25ms HAL-timeout). Tek satır `I2C1_Init GPIO_PULLUP` → **loop 32→6ms** (RD 26000→94µs), Ts/dt 0.16→**0.83**; timer-ISR rebuild gerekmedi. Dropout ÇÖZÜLDÜ (~940µF bulk; Sagemcom OCP). **✅ HP RE-TEST: gross stick-slip ÇÖZÜLDÜ** (6ms'te HER hedefte hareket; 90°→0.00° err FF ile; 32ms %95-stuck'tı — kıyas sağlam). **🔜 Sıradaki = Coulomb residual** (gross değil, tuning): FF-siz dead-band 3-6° ↔ FF limit-cycle trade-off; analitik seçenek A=pürüzsüz FF (tanh). ⚠ **rijit M3-mount bekliyor** — re-test gövde elle tutuldu → ince residual konfoundlu (gross sağlam). Tam detay → docs §12.13.4 + kronoloji ↓.
