...
**Şu an:** 🟡 **Aşama 3 (MIMO) AÇIK** (`feature/asama-3-mimo-model`, 2026-06-09). **3.1 pin planı ✅ onaylı + kablolama tamam; 3.2a encoder-2 bench PASS** (`artifacts/3/enc2_test/`); IMU kendini-iyileştirme firmware. **3.2b motor-2 sürücü firmware ✅** (minimal açık-döngü `Motor2_*`, `DUTY2:` komutu + `U2` telemetri, build PASS Flash %8.4; motor-2 stall + shared-struct refactor 3.3'e ertelendi — sertifikalı motor-1 dokunulmadı). **Sıradaki: 3.2b bench yön/kimlik testi** (`scripts/motor2_sign_test.py` — motor-2 döner mi + EC2 okur mu + polarite motor-1'e göre AYNI/TERS, 3.3 cascade geri-besleme işareti için) **→ baseline 2-eksen (3.3).** Aşama-2-sonrası paket main'de (`512e796`): mirror Kp_pos=6 gerçek 4.02°, amper bütçesi datasheet denetimi (3A yeterli, dar boğaz=sürücü), **count-tabanlı stall fix bench PASS** (yanlış-pozitif 0, oto-devam ~1 sn — 2A.T5-B kapandı), IMUDIAG/IMUINIT teşhis komutları. Önceki: ✅ Aşama 1+2 KAPALI (`asama-2-kapali` tag). Aşama 1: `K=53.89 rad/s/V, τ_median=60.5 ms, V_dead≈0, V_supply=12.15V`. Aşama 2.1: 5 kontrolcü tasarım. Aşama 2.2: firmware Tustin PI + anti-windup + MODE/SP_W komutları.
