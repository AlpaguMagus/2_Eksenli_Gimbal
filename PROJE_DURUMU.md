...
**Şu an:** 🟡 **Aşama 3 (MIMO) AÇIK** (`feature/asama-3-mimo-model`). Asimetrik donanım: **HP** (Motor1/HW-039/BTS7960, 20:1, yük) + **LP** (Motor2/TB6612, 9.7:1). **LP cascade çalışıyor** (K0 kapandı — Test 2.5/MIRROR/STAB). **HP karakterize + analitik cascade tasarlandı** (Kg=1043, τ≈70ms, forward temiz) ama **bench STICK-SLIP** → kök-neden **KANITLANDI: ~32ms IMU-bağlı loop hızlı HP'yi büyük stiction'la kontrol edemiyor** (kazanç-uzayı tüketildi: zayıf Ki=stuck, güçlü Ki=kararsız). Dropout ÇÖZÜLDÜ (~940µF bulk; Sagemcom OCP). **🔜 Sıradaki (1) = loop-rate fix:** loop ÖLÇÜLEN ~32ms; profil **26ms'i IMU okuması** (`MPU6050_Read` I2C) yiyor (§12.13.2). "7ms" git-arkeolojisiyle ÇÜRÜTÜLDÜ (hiç ölçülmedi, REGRESYON DEĞİL — §12.13.1). → (1) = **IMU read'i hızlandır** (neden tam 26ms? I2C clock-stretch/timeout teşhisi, datasheet-önce) → loop ~6ms → HP cascade çalışır; timer-ISR son çare. Tam detay → docs §12.11/§12.12/§12.13 + kronoloji ↓.
