#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HP dead-band — STATİK kopma vs KİNETİK dropout (iki yön, ≤0.5 cap).

Faz 1 ilk geçişte 1c aralığı (0.01-0.15) dardı; 1d motorun 0.20'de durduğunu 0.30'da döndüğünü
gösterdi → statik kopma ~0.25 (büyük stiction). Bu test ikisini AYIRIR:
  • STATİK kopma  : dinlenmeden yukarı ramp → ilk hareket duty'si (breakaway, stiction)
  • KİNETİK dropout: kick ile döndür → aşağı ramp → hareketin durduğu duty (Coulomb/kinetik)
Stiction ≫ kinetik = stick-slip; cascade dead-band telafisi + FF için kritik (§12.8 sınıfı).
⚠ FİZİKSEL, mil serbest, HP HW-039. Çıktı: artifacts/3/hp_deadband/<ts>/
"""
...
print("="*60); print(" HP DEAD-BAND — statik kopma vs kinetik dropout"); print("="*60)
send("STOP"); time.sleep(0.3); send("MODE:DUTY"); time.sleep(0.2); send("STALLEN:0"); time.sleep(0.2)
res={"static":{}, "kinetic":{}}

def static_breakaway(sign, lab):
    print(f"\n[STATİK] {lab} — dinlenmeden yukarı ramp:")
    vb=None
    for k in range(18,35):           # 0.18 .. 0.34
...
def kinetic_dropout(sign, lab):
    ...
    send(f"DUTYR:{clamp(sign*0.45)}"); t=time.time()
    while time.time()-t<1.2: send("PING"); send(f"DUTYR:{clamp(sign*0.45)}"); time.sleep(0.1)
    vd=None
...
d=f"artifacts/3/hp_deadband/{ts}"; os.makedirs(d,exist_ok=True)
status="PASS" if (sf and sr and kf and kr) else "PARTIAL"
json.dump({"test_id":"3.HP-deadband","timestamp":ts,"commit":commit(),"status":status,
...
with open(f"{d}/summary.md","w") as f:
    f.write(f"""# 3.HP-deadband — statik kopma vs kinetik dropout
...
- **Donanım:** HP HW-039, mil serbest, duty cap {DUTY_CAP}.
