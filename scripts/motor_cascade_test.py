#!/usr/bin/env python3
"""
Aşama 3.3 — Eksen cascade pozisyon step testi (axis-parametrik).

İnstance-based 2-eksen firmware'inde (g_axis[2]) bir eksenin cascade'ini
(pozisyon P dış → hız PI iç) gerçek motorda doğrular. position_step_test.py'nin
(Aşama 2.5) eksen-parametrik genellemesi — motor-1 (base komutlar) veya motor-2
('2' sonekli komutlar) seçilebilir.

ASIL SORU: Seçilen eksende cascade temiz oturuyor mu (hedefe ulaşma, overshoot,
limit-cycle yok)? Motor-2 = Aşama 1-2'de karakterize sağlıklı ünite → temiz PASS
beklenir (kazançlar zaten ona göre tasarlandı). Motor-1 = kusurlu ünite → CW
segmentlerinde takılma beklenir (3.x teşhisi, değişene kadar).

Akış (--motor 2 için):
  - MODE2:POS (firmware enc-2'yi 0° referans yapar)
  - KPP2:<kp_pos> (varsayılan 2.0)
  - Ardışık mutlak açı hedefleri: POS_DEG2:<deg>, her biri ~hold sn
  - θ_out = EC2 × 360/466 (çıkış mili derece)
  - Metrik (segment): ss_error, overshoot, settling (±2°), limit-cycle (θ std)

Çıktı (CLAUDE.md disiplini): artifacts/3/cascade_m<motor>/<test_id>/
    ├── raw/data.csv(.gz)   ├── summary.md   ├── meta.json   └── cascade_plot.png

Kullanım:
    python3 scripts/motor_cascade_test.py --motor 2
    python3 scripts/motor_cascade_test.py --motor 2 --targets 30 90 45 0 -45 0 --kpp 2.0 --hold 4

NOT: Serbest mil (yüksüz). Mekaniğe bağlıysa kazançlar yük altında yeniden
ayar ister (ROADMAP §5). Kaynaklar: [Franklin2010] §6.4 (cascade), §4.3 (tip-1 ss).
"""
...
def run_segment(ser, R, target_deg, hold_s, writer, seg_idx, t_global0):
    """Bir pozisyon hedefi. CSV'ye yaz, metrik döndür. R = derlenmiş regex/komut bağlamı."""
    send(ser, f"{R['pos_cmd']}:{target_deg}")
    print(f"[{ts()}] segment {seg_idx:2d}  {R['pos_cmd']}:{target_deg:+.0f}°", flush=True)
    ...
    while time.time()-t0 < hold_s:
        ...
        mec = R['ec'].search(line)
        if not mec: continue
        ec = int(mec.group(1))
        theta = ec * DEG_PER_COUNT
        mo=R['omega'].search(line); ms=R['sp'].search(line); mu=R['u'].search(line)
...
def compute_metrics(target, samples):
    ...
    t  = [s[0] for s in samples]; th = [s[1] for s in samples]
    th_start = th[0]; span = abs(target - th_start)
    tail = th[int(len(th)*0.6):]; th_ss = mean(tail)
    ss_err_deg = abs(th_ss - target)
    ss_err_pct = ss_err_deg/abs(span)*100 if span > 1 else 0.0
    if target >= th_start: peak = max(th); ov = max(0.0, (peak-target))
    else:                  peak = min(th); ov = max(0.0, (target-peak))
    ov_pct = ov/abs(span)*100 if span > 1 else 0.0
    settle_t = None
    for i in range(len(th)):
        if all(abs(v-target) <= SETTLE_BAND_DEG for v in th[i:]):
            settle_t = t[i]; break
    th_std_ss = std(tail); limit_cycle = th_std_ss > LIMIT_CYCLE_DEG
...
def build_ctx(motor):
    """Eksen-parametrik komut + telemetri-alan bağlamı.
       motor 1 → base komutlar + EC/OMEGA/SP/U ; motor 2 → '2' sonekli + EC2/OMEGA2/SP2/U2."""
    sfx = "" if motor == 1 else "2"
    return {
        "sfx": sfx,
        "mode_cmd": f"MODE{sfx}",
        "pos_cmd":  f"POS_DEG{sfx}",
        "kpp_cmd":  f"KPP{sfx}",
        "ec":    re.compile(rf"EC{sfx}:(-?\d+)"),
        "omega": re.compile(rf"OMEGA{sfx}:(-?[\d.]+)"),
        "sp":    re.compile(rf"SP{sfx}:(-?[\d.]+)"),
        # motor-1: ",U:"  (OMEGA'dan ayır); motor-2: "U2:" (yön+|duty|, signed)
        "u":     re.compile(r",U:(-?[\d.]+)") if motor == 1 else re.compile(r"U2:(-?[\d.]+)"),
    }


def main():
    ap=argparse.ArgumentParser(description=__doc__.split("\n\n")[0])
    ap.add_argument("--motor", type=int, default=2, choices=[1,2], help="hangi eksen (1=base, 2='2' sonekli)")
    ...
    args=ap.parse_args()
    R = build_ctx(args.motor)

    test_id = args.test_id or dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    out = Path(f"artifacts/3/cascade_m{args.motor}")/test_id
    (out/"raw").mkdir(parents=True, exist_ok=True)
    raw_csv = out/"raw"/"data.csv"

    print(f"[{ts()}] Eksen-{args.motor} cascade step — Kp_pos={args.kpp}, hedefler={args.targets}")
    print(f"[{ts()}] Komutlar: {R['mode_cmd']}:POS / {R['pos_cmd']}: / {R['kpp_cmd']}:")
    ...
    try:
        ...
        send(ser,f"{R['mode_cmd']}:POS"); time.sleep(0.1)     # enc 0° referans
        send(ser,f"{R['kpp_cmd']}:{args.kpp}"); time.sleep(0.05)
        ...
        with raw_csv.open("w",newline="") as fh:
            ...
            for i,tg in enumerate(args.targets,1):
                m=run_segment(ser, R, tg, args.hold, w, i, t_global0)
                results.append(m)
                if m.get("valid"):
        ...
        send(ser,"STOP"); send(ser,f"{R['mode_cmd']}:DUTY"); time.sleep(0.2)
    ...
    plot_path = make_plot(out, raw_final, args.motor)
    ...
    if valid and n_lc==0 and n_noreach==0:      status = "PASS"
    elif n_lc:                                   status = "LIMIT_CYCLE"
    elif n_noreach:                              status = "NO_REACH"   # motor dönmedi/takıldı — FAIL
    else:                                        status = "PARTIAL"
    write_artifacts(out, test_id, args, results, status, raw_final, plot_path)
...
def make_plot(out, raw, motor):
    ...
    tg=[float(r["t_global"]) for r in rows]; th=[float(r["theta_deg"]) for r in rows]
    om=[float(r["omega"]) for r in rows]; u=[float(r["u"]) for r in rows]; sp=[float(r["sp"]) for r in rows]
    tgt=[float(r["target_deg"]) for r in rows]
    fig,ax=plt.subplots(3,1,figsize=(11,8),sharex=True)
    ax[0].plot(tg,th,'b',lw=1.3,label='θ_out (ölçülen)'); ax[0].plot(tg,tgt,'r--',lw=1.0,label='hedef')
    ax[0].set_ylabel('θ çıkış mili (°)'); ax[0].legend(loc='best'); ax[0].grid(alpha=0.3)
    ax[0].set_title(f'Aşama 3.3 — Eksen-{motor} cascade pozisyon step (gerçek motor)')
    ax[1].plot(tg,sp,'m',lw=0.9,label='ω_ref (dış P çıkışı)'); ax[1].plot(tg,om,'b',lw=1.1,label='ω ölçülen')
    ...
    fig.tight_layout()
    p=out/"cascade_plot.png"; fig.savefig(p,dpi=130); plt.close(fig)
    return p


def write_artifacts(out, test_id, args, results, status, raw, plot):
    with (out/"summary.md").open("w") as f:
        f.write(f"# Aşama 3.3 — Eksen-{args.motor} Cascade Pozisyon Step\n\n")
        f.write(f"- **Test ID:** {test_id}\n- **Tarih:** {dt.datetime.now():%Y-%m-%d %H:%M}\n")
        f.write(f"- **Commit:** `{commit()}`\n")
        cmd_style = "base" if args.motor == 1 else "'2' sonekli"
        f.write(f"- **Eksen:** motor-{args.motor} ({cmd_style} komutlar)\n")
        ...
        f.write(f"- **Komut:** `python3 scripts/motor_cascade_test.py --motor {args.motor} --kpp {args.kpp}`\n")
        f.write(f"- **NOT:** Serbest mil (yüksüz). Mekanik+yük → ROADMAP §5 yeniden-ID.\n\n")
        ...
        f.write("- θ_out = EC×360/466 (çıkış mili, 0.773°/count). instance-based g_axis firmware.\n")
        f.write(f"\n## Artifacts\n- `{raw.name}`\n")
        if plot: f.write(f"- `{plot.name}` (θ takip + ω_ref/ω + u)\n")
    meta={"test_id":test_id,"title":f"Aşama 3.3 eksen-{args.motor} cascade step",
          "timestamp":dt.datetime.now().isoformat(timespec="seconds"),"commit":commit(),
          "status":status,"motor":args.motor,"architecture":"cascade_pos_P_speed_PI",
          "kp_pos":args.kpp,"inner":{"Kp":0.002,"Ki":0.1},"targets":args.targets,
          "note":"serbest mil; instance-based 2-eksen firmware (g_axis)",
