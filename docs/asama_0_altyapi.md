...
| α Değeri | Gyro Ağırlığı | Accel Ağırlığı | Zaman Sabiti ($\Delta t=0.008$ s) | Davranış |
|---|---|---|---|---|
| 0.99 | %99 | %1 | ~0.79 s | Çok düzgün, drift düzeltmesi yavaş |
| **0.98** | **%98** | **%2** | **~0.39 s** | **Dengeli (mevcut ayar)** |
| 0.95 | %95 | %5 | ~0.15 s | Accel gürültüsü belirginleşir |
| 0.80 | %80 | %20 | ~0.032 s | Accel'e çok bağımlı, gürültülü |

Zaman sabiti yaklaşık olarak $\tau \approx \Delta t\cdot\frac{\alpha}{1-\alpha}$. Kanonik döngü periyodu $\Delta t=8$ ms (IMU okunurken ~125 Hz; §5.4) ile mevcut ayarda $\tau \approx 0.008\cdot\frac{0.98}{0.02} \approx 0.39$ s, yani gyro drift'i yaklaşık 0.4 saniye içinde ivmeölçer referansına doğru düzeltilir.
...
Mevcut değerlerle ($\alpha=0.98$, kanonik döngü periyodu $\Delta t=8$ ms — IMU okunurken ~125 Hz, §5.4):

$$\omega_{co} = \frac{0.02}{0.98 \times 0.008} = 2.55\ \text{rad/s} \approx 0.41\ \text{Hz}$$

**Neden bu frekans makul?** Kesim, iki gürültü kaynağının kesiştiği yere yerleştirilmelidir:
- **Kesimin altında** (yavaş, $<0.41$ Hz): accel'e güven — yerçekimi mutlak referansı drift'siz; gyro burada drift biriktirir.
- **Kesimin üstünde** (hızlı): gyro'ya güven — accel burada titreşim/lineer-ivme gürültüsüne maruz.
- $\omega_{co}=2.55$ rad/s, motor kontrol bandının (cascade $\omega_c\approx1.9$ rad/s, [`00_genel_bakis.md`](00_genel_bakis.md) §2.6'daki $\sim0.3$ Hz cascade bandı; ayrıca asama_2 §11.13) **~1.34× üstünde** ama elle-sallama/titreşim bandının ($\gtrsim1$ Hz $\approx 6.28$ rad/s) altındadır. Yani accel füzyona yalnızca cascade bandı civarındaki yavaş hareketlerde sızar; insan elinin titreşim/sarsma bandı hâlâ gyro'ya bırakılır → kısa-vadeli dinamik korunur. Bu yüzden 0.98 **makul bir mühendislik seçimidir** (statik-optimal $\alpha_{opt}=0.9997$ değil; gerekçe aşağıda Allan variance bölümünde).

![Complementary filter frekans bölünmesi](../matlab/asama_0_altyapi/results/complementary_filter_bode.png)

**Şekil 5.2 —** Complementary filter frekans karakteristiği. Accel LPF (mavi) ve gyro HPF (kırmızı) kazançları $\omega_{co}=2.55$ rad/s'de kesişir; toplamları her frekansta 1'dir (tamamlayıcılık). Kesim, accel'in güvenilir olduğu düşük frekans ile gyro'nun güvenilir olduğu yüksek frekansı ayırır.
...
- **Lineer-ivme bozulması:** accel açısı sadece statikte doğru; α=0.9997 ile complementary düzeltme zaman sabiti $\tau=\Delta t\cdot\frac{\alpha}{1-\alpha}\approx 0.008\cdot\frac{0.9997}{0.0003}\approx 26.7$ s'ye uzar (kanonik $\Delta t=8$ ms; bu $\tau$, aşağıdaki $\tau_{co}=171$ s Allan-kesişim zaman sabitinden **ayrı** bir büyüklüktür) → uzun gimbal kullanımında drift birikir. α=0.98 ($\tau\approx0.39$ s) bu drift'i hızla bağlar.
...
> **Kanonik döngü periyodu (bir bakışta):** IMU aktif-okunurken **~8 ms (~125 Hz)** — bu projede `dt` için kullanılan referans değer. Diğer rakamlar özel durumlardır: **6 ms (~167 Hz)** = IMU-NACK durumu; **32 ms** = kopuk-IMU I2C-BUSY-timeout **artefaktı** (`GPIO_PULLUP` fix ile giderildi); eski "~7 ms / ~140 Hz" hiç ölçülmedi, bir **varsayımdı**. Aşağıdaki ayrıntı bu tarihçeyi açar.
