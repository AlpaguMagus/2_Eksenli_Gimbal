...
Reset Handler (startup_stm32f411xe.s)
  ...
  │
  └─► while(1) — Ana Döngü (bloklamayan; Aşama 0-2: ~7 ms / ~140 Hz, DWT-doğrulanmış → Aşama 3 bench: ÖLÇÜLEN ~32 ms / ~31 Hz, §5.4 + asama_3 §12.12.5)
...
Ana döngü **bloklamayan** (`HAL_Delay`'siz) bir süper-loop olarak çalışır; `dt`, milisaniye çözünürlüklü `HAL_GetTick()` yerine **DWT cycle counter** (96 MHz) ile mikrosaniye hassasiyetinde ölçülür — çünkü ms çözünürlük Aşama 0-2 döneminin ~7 ms'lik döngüsünde ±%14 jitter veriyordu (`[ARM_DWT]`). (Aşama 3 bench'te döngü ~32 ms'e yavaşladı, aşağıdaki not; DWT'nin µs-hassasiyet gerekçesi her iki rejimde de geçerli.):
...
- **Ana döngü — Aşama 0-2: ~7 ms (~140 Hz)** (DWT ile doğrulanmıştı) **→ Aşama 3 bench: ÖLÇÜLEN ~32 ms (~31 Hz)** (`asama_3_mimo_model.md` §12.12.5): sensör oku → filtre → kontrol → her iterasyonda; DWT ile ölçülür, `dt` fallback 5 ms. ⚠ Loop **yavaşladı** (silinmedi — niteleyiciyle işaretli); neden açık: ~27 ms IMU okuma/işleme (`src/main.c:409`). De-rating: $T_s/dt$ 0.71 (eski 7 ms) → 0.16 (güncel 32 ms). Loop-bağımlı türev sayılar (18.7 rad/s kuantizasyon, MA gecikmesi, faz marjı) 7 ms dönemine ait — 32 ms'te kayar.
...
> **Not (tutarlılık):** Eski tasarımda ana döngü `HAL_Delay(50)` ile ~20 Hz hedefliyordu; mevcut firmware DWT-zamanlı bloklamayan döngüye geçti (jitter düzeltmesi, Aşama 2.3). Belgedeki "~20 Hz" eski değerler bu DWT zamanlamasıyla (Aşama 0-2'de kontrol ~140 Hz, telemetri ~40 Hz) güncellenmişti. ⚠ **Güncelleme (Aşama 3):** kontrol döngüsü bench'te ~32 ms / ~31 Hz ölçüldü (yukarıdaki regresyon notu, `asama_3_mimo_model.md` §12.12.5) — "~140 Hz" yalnız Aşama 0-2 dönemine aittir, güncel değildir.
