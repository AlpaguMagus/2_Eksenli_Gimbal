...
Reset Handler (startup_stm32f411xe.s)
  ...
  │
  └─► while(1) — Ana Döngü (ÖLÇÜLEN ~32 ms / ~31 Hz; '7ms/140Hz' VARSAYIMDI, hiç ölçülmedi — §5.4 + asama_3 §12.13.1)
...
- **Ana döngü — ÖLÇÜLEN ~32 ms (~31 Hz)** (`asama_3_mimo_model.md` §12.13): sensör oku → filtre → kontrol → her iterasyonda; DWT ile ölçülür, `dt` fallback 5 ms. ⚠ **"~7 ms (~140 Hz)" iddiası ÇÜRÜTÜLDÜ** (git-arkeolojisi §12.13.1): hiç ölçülmedi, `HAL_Delay(5)` nominalinden varsayımdı — loop muhtemelen HEP ~32 ms, **REGRESYON DEĞİL**. Profil: ~26 ms IMU okuma (`MPU6050_Read`) + 5 ms `HAL_Delay` (§12.13.2). De-rating $T_s/dt = 0.16$. Loop-bağımlı türev sayılar (18.7 rad/s, MA gecikmesi, faz marjı) varsayılan-7 ms'e aitti — 32 ms'te kayar.
