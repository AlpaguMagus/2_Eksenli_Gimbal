...
> Y0 ilk bench-fit tamamlandı ($a=0.23$ SOLID R²=0.963, sürtünme 0.06/0.03 FIRST-CUT — §12.5.7); rafine
> ($\omega_n/\zeta$ temiz, sürtünme magnitüd) + Y1 cascade re-tune açık. **Kontrolcü yeniden tasarımı**
> firmware'de yazılı, **bench-validasyon** Y1 sonrası. Ad-hoc dönemin firmware/script kalıntıları kod-review
> ile temizlendi (§12.5.6).
...
Firmware mevcut: `kff_coul` fwd / `kff_coul_rev` rev (`LFFC:` komutu). Gerçek Y0: + yön ~0.06, − yön ~0.03
(FIRST-CUT magnitüd, yön/asimetri 2.16 solid; eski tek-açı 0.09/0.05 FAZLA tahminliydi — §12.5.7 fit_report).
Bu, ölü-bölgeyi besleme-ileri ile geçer → kontrolcü ince ayar yapabilir hale gelir. + gravite FF
($k_{ff,grav}=0.23$, Y0 gerçek fit §12.5.7). Mevcut cascade ($K_{p,pos}=2$) + bu FF ile bench-test edilecek.
...
**Gerçek bench fit sonucu (Y0 ilk koşum):** `loaded_plant_id_capture.py` (B1 üçgen-rampa + B3 validasyon)
koşuldu (test-id `20260624_085025`) → aynı estimator ile gerçek veriye fit (`loaded_plant_id_fit.m`).
Sonuç `loaded_motor_params.json` + `loaded_fit_report.md`'ye yazıldı:

| Parametre | Değer | Durum |
|---|---|---|
| $a$ (gravite, duty) | **0.23** | ✅ SOLID — 3-yol çapraz-doğrulama (B1 rampa 0.230; B3 0.20@64° → 0.222; B3 −0.11@−29° → 0.224) |
| $s_+$ (statik sürt. +, duty) | 0.059 | ⚠ yön solid, magnitüd FIRST-CUT |
| $s_-$ (statik sürt. −, duty) | 0.027 | ⚠ yön solid, magnitüd FIRST-CUT |
| asimetri $s_+/s_-$ | 2.16 | ✅ yön solid |
| B1 fit R² | 0.963 | ✅ |
| $\omega_n$ (rad/s) | 4.0 | ⚠ PRIOR (bu-oturum free-decay ring-down yapmadı) |
| $\zeta$ | — | belirsiz (free-decay Coulomb-biased; driven-step overshoot YOK) |

Stiction magnitüdü FIRST-CUT: stick-slip geniş statik band → settle band-içinde tek-değer değil → magnitüd
band-scatter'ıyla belirsiz; $a$ ise band-ORTASINDAN geldiği için sağlam. Eski tek-açı ID 0.09/0.05 graviteyi
sürtünmeye karıştırıp FAZLA tahmin etmişti — çok-açılı kopma + midpoint-vs-$\sin\theta$ ayrıştırması düzeltti.

![Y0 gerçek bench fit (B1 histerezis)](../matlab/asama_5_gimbal/results/loaded_plant_id/real_fit_085025.png)

> 📊 **Üreten betik:** `matlab/asama_5_gimbal/loaded_plant_id_fit.m`

**Sonraki (Y0-rafine + Y1):** sürtünme magnitüd precision (yarı-statik settled multi-açı adımlar, band-ortası
temiz ölçüm) + fresh $\omega_n/\zeta$ (temiz tek-büyük-bırakış free-decay veya driven-τ) + IMU payload'a taşı
(şu an base = yasa-demosu); ardından Y1 analitik kontrol (gravite-FF $a$ + sürtünme-FF + cascade re-tune).
